...
grid on;
legend('Airfoil Shape', 'Locations of Panel Joints')
hold off;
...
grid on;
legend('Airfoil Shape', 'Locations of Panel Joints','Camber Line')
hold off;
...
% Plotting the convergence of cl
figure();
plot(N_test_tot, cl_values, 'b-','LineWidth', 2);
hold on;

% Horizontal line for the "exact" solution with the large number of panels
yline(cl_exact, 'r--','LineWidth', 2)

% Horizontal lines for values within +/- 1% of the "exact" solution
error_bounds_1 = cl_exact*1.01;
error_bounds_2 = cl_exact*0.99;

yline(error_bounds_1, 'm--','LineWidth', 2)
yline(error_bounds_2, 'm--','LineWidth', 2)

% Vertical line to show the total number of panels to get within +/- 1% of
% the "exact" solution
xline(N_tot_1percent,'g--','LineWidth', 2)
    
xlabel('Number of Panels');
ylabel('Coefficient of Lift (c_l)');
title(sprintf('Number of Panels to Converge within 1%% of the "exact" c_l (N = %d total panels)', N_tot_1percent))
grid on;
legend('Converging c_l', '"Exact Solution"', '+1% "Exact Solution"','-1% "Exact Solution"', ...
    'Number of Panels that Achieves a C_l within 1%','Location', 'best')
hold off;

% Creating a table comparing the "exact" solution to our predicted solution
f = figure();

data = {2*N_exact,cl_exact,0;N_tot_1percent, cl_1percent, error_1percent};
colnames = {'Total Panels', 'c_l', 'Relative Error (%)'};
rownames = {'"Exact" Solution', 'Minimum for 1% Error'};

uitable(f,'Data', data, 'ColumnName', colnames,'RowName', rownames, ...
        'Position', [20 20 450 72]);